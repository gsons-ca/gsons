    ...
    <header className="relative bg-white border-b border-gray-200 font-sans z-50">
      <nav className="max-w-[1400px] mx-auto px-6 h-16 flex justify-between items-center">
        ...
        <div className="flex items-center space-x-8">
          ...
          {/* Desktop Navigation */}
          <div className="hidden lg:flex space-x-6 text-[15px] font-medium text-gray-700">
            {Object.keys(megaMenuContent).map((item) => (
              <button 
                key={item}
                onClick={() => toggleMenu(item)}
                className={`hover:text-green-600 pb-1 border-b-2 transition-colors ${
                  activeMenu === item ? 'border-green-600 text-green-600' : 'border-transparent'
                }`}
              >
                {item}
              </button>
